...
# Implémentation conforme aux spécifications du Prompt Professionnel
# =============================================================================
...
class TPValidator:
    ...
    def verify_results(self, rotor, modal=None, camp=None, tp_config=None) -> dict:
        ...
        if tp_config and "validation" in tp_config:
            ...
            if "expected_mass_min" in val:
                ...
                if m_ok:
                    ...
                    results["details"].append(("⚠️", f"Masse {rotor.m:.2f} kg hors plage [{val['expected_mass_min']}, {val['expected_mass_max']}]"))
...
{fn_table}
<div class='footer'>Généré par RotoPédago v5.0 — ROSS Rotordynamic Open-Source Software</div>
</body></html>"""
...
def render_homepage():
    ...
    <div style='text-align:center; padding: 30px 0 10px 0'>
      ...
      <p style='font-size:1.2em; color:#555'>Application pédagogique de rotordynamique • Propulsée par ROSS</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <p>✅ 5 analyses disponibles<br>✅ Bibliothèque de paliers<br>✅ Tableaux modaux</p>
        </div>
        """, unsafe_allow_html=True)
    with col3:
        st.markdown("""
...
def render_tp_mode():
    ...
    # ── SIMULATION ──
    with tab_sim:
        rotor, modal, camp, unbal = None, None, None, None
    ...
    with tab_valid:
        ...
        if rotor is None:
            ...
            val_result = validator.verify_results(rotor, modal=modal, tp_config=tp)
    ...
    with tab_report:
        ...
        if st.button("📥 Générer le rapport HTML"):
            val_result_rep = validator.verify_results(rotor, modal=modal, tp_config=tp) if rotor else {"details": [], "percentage": 0}
            html = reporter.generate_html(tp_id, params_used, val_result_rep, rotor, modal)
            st.download_button(
...
def _tp12_interface(tp):
    ...
    if modal:
        ...
        mode_idx = st.selectbox("Mode à visualiser :", range(min(n_modes, len(modal.evalues)//2)),
            ...
            try:
                for i in range(min(4, camp.log_dec.shape[1])):
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        y=ld, name=f"Mode {i+1}", line=dict(color=color if i == 0 else None)
                    ))
            except:
                pass
    ...
    if rotor:
        ...
        with tabs[1]:
            ...
            if modal:
                ...
                try:
                    st.plotly_chart(safe_plot(modal, mode=m_idx)),
 use_container_width=True)
        ...
        with tabs[3]:
            ...
            if camp_stab:
                fig_s = go.Figure()
                try:
                    for i in range(min(6, camp_stab.log_dec.shape[1])):
                        ld_vals = camp_stab.log_dec[:, i]
                        fig_s.add_trace(go.Scatter(
                            x=np.linspace(0, v_max, 100), y=ld_vals, name=f"Mode {i+1}"
                        ))
                    fig_s.add_hline(y=0, line_dash="dash", line_color="red")
                    fig_s.add_hline(y=0.1, line_dash="dot", line_color="orange",
                                    annotation_text="Seuil API (0.1)")
...
def render_documentation():
    ...
    with tab1:
        ...
        L'énergie cinétique et l'énergie de déformation intègrent les effets de :
        - **Cisaillement transversal** (κ = facteur de forme de section)
        ...
        st.markdown("""
        Où :
        - **M** = matrice de masse (translations + rotations)
        - **G** = matrice gyroscopique (fonction de la vitesse Ω)
        - **C** = matrice d'amortissement (paliers + amortissement structurel)
        - **K** = matrice de rigidité (arbre + paliers)

        ### Analyse Modale
        """)
        st.latex(r"\det(K - \lambda M) = 0 \quad \Rightarrow \quad \lambda_k = -\xi_k\omega_{nk} \pm j\omega_{dk}")
        st.markdown("""
    ...
    with tab2:
        ...
        |---------|-------------|--------------|
        | `run_static()` | Déflexion statique (gravité) | `plot_deflected_shape()`, `plot_bending_moment()` |
        | `run_modal(speed)` | Fréquences propres | `.wn`, `.wd`, `.log_dec`, `plot_mode_3d()` |
        | `run_campbell(speeds)` | Diagramme de Campbell | `.plot()`, `.log_dec` |
        | `run_critical_speed()` | Vitesses critiques | Tableau numérique |
        | `run_unbalance_response()` | Réponse fréquentielle balourd | `plot_magnitude()`, `plot_phase()` |
        | `run_freq_response()` | Réponse fréquentielle générale | `plot_bode()` |
        | `run_time_response()` | Transitoires temporels | Orbites, DFFT |
    ...
    with tab3:
        ...
        ### API 684 — 2nd Edition
        Standard API pour la rotordynamique des turbomachines :

        1. **Marge de vitesse critique ≥ 15%** : aucune vitesse critique dans [0.85·Nop, 1.15·Nop]
        2. **Log Dec ≥ 0.1** pour tous les modes dans la plage opérationnelle
        3. **Réponse au balourd** : amplitudes < limites ISO 7919
        4. **Stabilité à l'instabilité** : calcul du niveau d'instabilité résiduelle

        ### ISO 1925 — Terminologie
        Définitions officielles : balourd, centre de gravité, axe principal d'inertie,
        déséquilibre résiduel, qualité G.
        """)
...
# Arbre (5 éléments de 200mm, Ø50mm)
...
# Paliers aux extrémités (kxx=kyy=1e7 N/m, cxx=cyy=500 N·s/m)
bear0 = rs.BearingElement(n=0, kxx=1e7, kyy=1e7, kxy=0, kyx=0, cxx=500, cyy=500)
bear5 = rs.BearingElement(n=5, kxx=1e7, kyy=1e7, kxy=0, kyx=0, cxx=500, cyy=500)
...
fn = modal.wn / (2 * np.pi)
print("Fréquences propres (Hz):", fn[:4].round(2))
print("Log Décrément:", modal.log_dec[:4].round(4))
