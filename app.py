...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="RotoPédago v4.3 Pro", layout="wide")
...
# --- 3. NAVIGATION ---
st.sidebar.title("🚀 RotoPédago v4.3")
...
if mode == "🏗️ Constructeur Libre":
    ...
    with col1:
        ...
        ed_s = st.data_editor(df_s, num_rows="dynamic", key="s_v43")

    with col2:
        st.subheader("2. Disques (Disks)")
        df_d = pd.DataFrame([{"node": 2, "id": 0.05, "od": 0.25, "width": 0.07}])
        ed_d = st.data_editor(df_d, num_rows="dynamic", key="d_v43")
    ...
    with col3:
        ...
        ed_b = st.data_editor(df_b, num_rows="dynamic", key="b_v43")
    ...
    try:
        ...
        with t3:
            ...
            try:
                ...
                fig_stab.update_layout(xaxis_title="Vitesse (RPM)", yaxis_title="Log Decrement")
        ...
        with t4:
            ...
            try:
                ...
                # TEST DE COMPATIBILITÉ AUTOMATIQUE DES FONCTIONS
                fig_static = None
                for method_name in ['plot', 'plot_deformation', 'plot_deflected_shape']:
                    if hasattr(static_res, method_name):
                        fig_static = getattr(static_res, method_name)()
                        break
                
                if fig_static:
                    st.plotly_chart(fig_static, use_container_width=True)
                else:
                    st.error("Méthode d'affichage statique introuvable.")
        ...
        with t5:
            ...
            fig_modal = None
            for method_name in ['plot_mode_3d', 'plot_mode_shape', 'plot']:
                if hasattr(modal, method_name):
                    try:
                        fig_modal = getattr(modal, method_name)(mode=m_idx)
                        break
                    except: continue
    ...
    st.write("Faites varier Kxy pour observer l'instabilité (Log Dec < 0).")
    k_excit = st.slider("Coefficient croisé Kxy (N/m)", 0.0, 1e7, 1e6, step=1e6)
...
st.sidebar.caption("Stable Version 4.3 - Auto-compatible")
